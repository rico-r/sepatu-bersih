                    ...
                    <table class="table table-bordered" id="dataPesanan" width="100%" cellspacing="0">
                        ...
                        @foreach ($listPesanan as $pesanan)
                            <tr>
                                ...
                                <td>{{ formatDatetime($pesanan->detail_status->where('status', 'done')->first()->created_at) }}</td>
                                <td>
                                    <a class="btn btn-primary" title="Lihat detail pesanan" href="{{ route('order.view', ['pesanan' => $pesanan->id] )}}">
                                        <i class="fa fa-eye"></i>
                                    </a>
                                    <a class="btn btn-danger" title="Batalkan diambil" onclick="revertDone(this, {{ $pesanan->id }})">
